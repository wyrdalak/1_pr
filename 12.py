...
class FaceRecognitionApp:
    def __init__(self):
        ...
        # last time overcrowd warning was logged
    ...
    def _count_active_permits(self, env_id: str) -> int:
        """Return number of active access permits for the environment."""
        now = datetime.datetime.now()
        cnt = 0
        for rec in self.assignments:
    ...
    def _update_security_frame(self):
        ...
        # check overcrowding (more people present than issued permits)
        if self.security_env_id:
            current_count = len([n for n in present if n != 'Неизвестный'])
            permits = self._count_active_permits(self.security_env_id)
            if current_count > permits and time.time() - self.last_overcrowd_log > 30:
                self.last_overcrowd_log = time.time()
                env = next((e for e in self.environments if e.get('id') == self.security_env_id), {})
                env_name = env.get('name', 'Неизвестное помещение')
                self._send_log('WARNING',
                               f'В {env_name} сотрудников больше, чем выдано допусков ({current_count}/{permits})')
