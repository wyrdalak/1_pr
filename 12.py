...
def save_environments(envs):
    with open(ENVIRONMENT_FILE, "w", encoding="utf-8") as f:
        for e in envs:
            f.write(f"{e['name']};{e['location']};{e['image']}\n")
...
class FaceRecognitionApp:
    ...
    def _build_admin_env_frame(self):
        ...
        ttk.Button(f, text="Добавить", command=self._add_environment).pack(pady=10)
    ...
    def _add_environment(self):
        ...
        self.env_status.config(text=f"Помещение {name} сохранено")
        self.env_name_entry.delete(0, 'end')
        self.env_loc_entry.delete(0, 'end')
        self.env_selected_image = ""
        self.env_image_label.config(text="")
    ...
    def _refresh_env_catalog(self):
        ...
        for idx, env in enumerate(self.environments):
            ...
            try:
                ...
                pass
            txt = f"{env['name']} ({env['location']})"
            ttk.Label(rec, text=txt, style='Status.TLabel').pack(side='left', padx=10)
            ttk.Button(rec, text='Удалить', command=lambda i=idx: self._delete_environment(i)).pack(side='right', padx=5)

    def _delete_environment(self, idx):
        if 0 <= idx < len(self.environments):
            del self.environments[idx]
            save_environments(self.environments)
            self._refresh_env_catalog()
