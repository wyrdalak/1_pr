...
class FaceRecognitionApp:
    ...
    def _build_manager_frame(self):
        ...
        self._apply_gradient_background(canvas_frame)
        toolbar = ttk.Frame(canvas_frame)
        toolbar.place(relx=0.5, rely=0, anchor='n')
        self.zone_tool_buttons = {}
        btn_opts = {'width': 32, 'height': 32, 'activebackground': '#d0d0ff'}
    ...
    def _set_zone_tool(self, tool):
        ...
        if hasattr(self, 'zone_tool_buttons'):
            for name, btn in self.zone_tool_buttons.items():
                ...
                btn.config(relief=relief, bg=bg, activebackground=bg)
